...
if __name__ == "__main__":
    ...
    eval_clf = RandomForestClassifier(n_estimators=150, max_depth=50, random_state=1126)
    ...
    created_df = data.duplicate_data((2016, 6, 1), (2017, 3, 31), ratio=1)
    # created_df = data.create_data((2016, 6, 1), (2017, 3, 31), ratio=1, offset=5)
    augmented_df = pd.concat([train_df, created_df[train_df.columns]], axis=0)
    y_df = augmented_df[["adr", "is_canceled"]]
    X_df = augmented_df.drop(["adr", "is_canceled"], axis=1)

    reg = HistGradientBoostingRegressor(random_state=1126)
    reg.fit(X_df.to_numpy(), y_df["adr"].to_numpy())
    clf = RandomForestClassifier(n_estimators=150, max_depth=50, random_state=1126)
